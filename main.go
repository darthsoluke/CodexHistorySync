...
func run() int {
	...
	flag.StringVar(&codexHomeFlag, "codex-home", "", "Codex home directory. Defaults to the first detected local Codex home.")
	flag.StringVar(&stateDBFlag, "state-db", "", "Explicit Codex state database path.")
	flag.StringVar(&providerFlag, "provider", "", "Target provider name. Defaults to model_provider in config.toml.")
	flag.Var(&fromProviders, "from-provider", "Only retag threads from these providers. Repeatable. Default: all providers except the target provider.")
	flag.StringVar(&backupDirFlag, "backup-dir", "", "Backup directory. Default: <codex-home>/backups/relink-<timestamp>.")
	flag.BoolVar(&applyFlag, "apply", false, "Write changes. Without this flag, the tool only prints a preview.")
	flag.Usage = func() {
